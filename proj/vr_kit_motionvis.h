...
#include <libs/cgv_gl/rounded_cone_renderer.h>
...
public:
	...
	///
	bool render_start_drawing = true;
	/// set this
...
public:
	...
	vr_kit_motionvis()
	{
		...
		connect(get_animation_trigger().shoot, this, &vr_kit_motionvis::timer_event);
	}
	/// call this
	...
	void render_trajectory(context& ctx) {
		if (!(vr_view_ptr && render_start_drawing))
			return;
